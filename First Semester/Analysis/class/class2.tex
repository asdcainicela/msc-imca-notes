...
\thmr{Equivalencia de normas en \( \mathbb{R}^n \)}{teorema6}{
	...
	Es decir, dadas dos normas \( \|\cdot\|_a \) y \( \|\cdot\|_b \) en \( \mathbb{R}^n \), existen constantes \( \alpha, \beta \in \mathbb{R}^+ \) tales que:
	\[
	\forall\, x \in \mathbb{R}^n,\quad \alpha \|x\|_a \leq \|x\|_b \leq \beta \|x\|_a.
	\]
}
...
\cor{
	Sean $(x_k)_{k\in \N} $ y $(y_k)_{k\in \N} $ sucesiones en $\Rn$ tal que
	$$
	\lim\lim_{k \to \infty} x_k = a \in \Rn \text{ y } \lim\lim_{k \to \infty} y_k = b \in \Rn
	$$
	Sea  $(\lambda_k)_{k\in \N} \subset \R  $ tal que $	\lim\lim_{k \to \infty} \lambda_k = \lambda_0$
	...
	\begin{description}
		...
		\item[(ii) ]
		\begin{equation}
			\lim_{k \to \infty} \lambda_k x_k = \lambda_0 	\lim_{k \to \infty}  x_k= \lambda_0 \cdot a
		\end{equation}
		...
		\end{equation}
		\item[(iv)  Para toda norma]
		\begin{equation}
			\lim_{k \to \infty} \norm{x_k} = \norm{\lim_{k \to \infty} x_k } =\norm{a}
		\end{equation}
	\end{description}
	donde, $a=(a_1, \ldots, a_n), \quad b=(b_1, \ldots, b_n),\quad x_k = (x_1^k, \ldots, x_n^k)$ e $y_k = (y_1^k, \ldots, y_n^k)$
